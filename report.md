...
fine-tuning 없이, 두 baseline 모델이 masked 입력에서 실제로 어떤 이미지를 상위로 가져오는지 두 쿼리로 확인했다.

![baseline query 644046](figures/02_baseline/baseline_query_644046.jpg)
*그래픽 티셔츠 원피스 쿼리. 두 모델 모두 같은 그래픽/텍스트 프린트 원피스를 상위로 가져온다 — 실루엣(짧은 소매, 박시한 핏)보다 프린트 패턴에 더 반응하는 경향이 보인다. top-5 cosine은 SigLIP2 0.79~0.82, Tianmu-MERE 0.72~0.77 수준.*

![baseline query 437266](figures/02_baseline/baseline_query_437266.jpg)
*체크무늬 셔츠 원피스 쿼리. 이 경우엔 두 모델 다 실루엣이 비슷한 셔츠형/티어드 원피스를 상위로 가져와(top-5 IoU 0.73~0.90), 실루엣 신호가 이미 어느 정도 반영되고 있음을 보여준다. 두 사례를 같이 보면 "완전히 못 잡는다"가 아니라 "패턴이 강하면 패턴에 끌려가고, 패턴이 약하면 실루엣이 드러난다"는 쪽에 가깝다.*

마스크 처리된 입력에서는 배경·색상 정보가 없어지므로 실루엣 신호가 상대적으로 더 부각되지만, 프린트/텍스트 패턴 자체는 마스크 안에 그대로 남아 있어 여전히 강하게 작용한다.
